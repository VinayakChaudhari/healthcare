...
const PatientsLogin = (props) => {
    ...
    function sendLoginRequest(){
        ...
        .loginStudent(studOb)
        .then((response) => {
          console.log('patients validated successfully ', response.data);
         // history.push('/category');
        //  console.log('id of the customer', response.data.id);
         setStudId(response.data.id);

         console.log('id of the patients', response.data.id);
         console.log('id of the patients', studId);
         setLoginFlag(false);
        //  navigate(`/customer`);
    ...
    return (
        ...
        {loginFlag?(
            ...
            <Patient studentId={studId}></Patient>
            };
